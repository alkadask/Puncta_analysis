...
while (keepgoing == "yes"){
	...
	run("RGB Stack");			//convert RGB image to a stack of separated red, green and blue channels
	run("Delete Slice");		//delete red channel since this channel is blank
	run("Next Slice [>]");
	run("Delete Slice");		//delete blue channel since this channel is blank
	...
	roiManager("Select", 0);
	run("Enlarge...", "enlarge=30");	//enlarge selected areas by a certain number of pixel radius. this operation effectively joins the puncta in the neuron to get a single area selection spanning the entire neuron
	roiManager("Add");
	roiManager("Select", 1);
	roiManager("Split");
	run("Select None");
	...
	saveAs("Tiff", fpath+"/Processed_files/"+mask);
	...
	//add the spline selection to the original image
	selectWindow(name);
	Roi.setPolylineSplineAnchors(xpoints, ypoints)
	roiManager("Add");
	roiManager("Select", 0);
	...
	roiManager("update");
	roiManager("save selected", fpath+"/Processed_files/"+name+".roi");
	
	//straighten segemented line
	run("Straighten...", "line=20");
	straightened = replace(name, ".tif", "_straightened.tif");
	saveAs("Tiff", fpath+"/Processed_files/"+straightened);
